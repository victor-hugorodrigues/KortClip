...
# ---- WebUI Config Saving ----
WEBUI_CONFIG_PATH = os.path.join(WORKING_DIR, "webui_config.json")
...
def get_config(key, default_value):
    return WEBUI_CONFIG.get(key, default_value)
...
def run_viral_cutter(input_source, project_name, url, start_time, end_time, video_file, segments, viral, themes, min_duration, max_duration, model, ai_backend, api_key, ai_model_name, chunk_size, workflow, face_model, face_mode, face_detect_interval, no_face_mode, 
    ...
    if input_source == "Existing Project":
        ...
        if start_time: cmd.extend(["--start-time", start_time])
        if end_time: cmd.extend(["--end-time", end_time])
        # Pass Video Quality
...
input[type="text"], input[type="password"], textarea, select, .gr-input, input[type="number"], .wrap.svelte-1f4lxsf {
...
/* Focus States */
input:focus, textarea:focus, select:focus, .gr-input:focus, .wrap.svelte-1f4lxsf:focus-within {
    border-color: #E11D48 !important;
    box-shadow: 0 0 12px rgba(225, 29, 72, 0.3) !important;
    background-color: rgba(15, 15, 20, 0.95) !important;
    outline: none !important;
}

/* Labels */
.gr-form > label, span.text-gray-500, .label-wrap > span {
...
.wrap.svelte-1f4lxsf > ul.options, .options.svelte-1kxjx3f {
...
.options.svelte-1kxjx3f > li, .wrap.svelte-1f4lxsf > ul.options > li {
...
.options.svelte-1kxjx3f > li:hover, .options.svelte-1kxjx3f > li.selected, .wrap.svelte-1f4lxsf > ul.options > li:hover, .wrap.svelte-1f4lxsf > ul.options > li.selected {
...
.tabs > div.tab-nav > button:hover:not(.selected) {
    color: #E2E8F0 !important;
}
...
label.radio, label.checkbox, .gr-form fieldset label, .wrap > label {
...
label.radio:hover, label.checkbox:hover, .gr-form fieldset label:hover, .wrap > label:hover {
    background-color: rgba(255, 255, 255, 0.1) !important;
}

label.radio.selected, label.checkbox.selected {
    border-color: #E11D48 !important;
...
custom_theme = gr.themes.Base(
    ...
    background_fill_primary="#0C0C12",
    background_fill_primary_dark="#0C0C12",
    background_fill_secondary="#05050A",
    background_fill_secondary_dark="#05050A",
    border_color_primary="rgba(255,255,255,0.05)",
    border_color_primary_dark="rgba(255,255,255,0.05)",
    block_background_fill="rgba(12,12,18,0.6)",
    block_background_fill_dark="rgba(12,12,18,0.6)",
    block_border_color="rgba(255,255,255,0.04)",
    block_border_color_dark="rgba(255,255,255,0.04)",
    block_label_background_fill="transparent",
    block_label_background_fill_dark="transparent",
    block_label_text_color="#94A3B8",
    block_label_text_color_dark="#94A3B8",
    input_background_fill="rgba(5,5,10,0.8)",
    input_background_fill_dark="rgba(5,5,10,0.8)",
...
with gr.Blocks(title=i18n("KortClip WebUI"), theme=custom_theme, css=css) as demo:
        ...
        with gr.Tab("✨ " + i18n("Create New")):
             with gr.Row():
                with gr.Column(scale=1):
                    ...
                    with gr.Row(visible=True) as time_interval_row:
                         start_time_input = gr.Textbox(label=i18n("Start Time (Optional)"), placeholder="e.g. 00:00:00", visible=True)
                         end_time_input = gr.Textbox(label=i18n("End Time (Optional)"), placeholder="e.g. 01:23:45", visible=True)
                    ...
                    def on_source_change(source):
                        if source == "YouTube URL":
                            return gr.update(visible=True), gr.update(visible=True), gr.update(visible=False), gr.update(visible=False), gr.update(value="Full") 
                            ...
                            return gr.update(visible=False), gr.update(visible=False), gr.update(choices=projs, visible=True), gr.update(visible=False), gr.update(value="Subtitles Only")
                    
                    
                    with gr.Row():
                        segments_input = gr.Number(label=i18n("Segments"), value=get_config("segments", 3), precision=0)
                        viral_input = gr.Checkbox(label=i18n("Viral Mode"), value=get_config("viral", True))
                    themes_input = gr.Textbox(label=i18n("Themes"), placeholder=i18n("funny, sad..."), value=get_config("themes", ""), visible=False)
                    viral_input.change(lambda x: gr.update(visible=not x), viral_input, themes_input)
                    with gr.Row():
                        min_dur_input = gr.Number(label=i18n("Min Duration (s)"), value=get_config("min_dur", 15))
                        max_dur_input = gr.Number(label=i18n("Max Duration (s)"), value=get_config("max_dur", 90))
                with gr.Column(scale=1):
                    with gr.Row():
                        ai_backend_input = gr.Dropdown(choices=[(i18n("Gemini"), "gemini"), (i18n("G4F"), "g4f"), (i18n("Local (GGUF)"), "local"), (i18n("Manual"), "manual")], label=i18n("AI Backend"), value=get_config("ai_backend", "gemini"), scale=2)
                        api_key_input = gr.Textbox(label=i18n("Gemini API Key"), type="password", value=get_config("api_key", ""), scale=3)
                    
                    # New Dynamic Inputs
                    with gr.Row():
                        ai_model_input = gr.Dropdown(choices=GEMINI_MODELS, label=i18n("AI Model"), value=get_config("ai_model", GEMINI_MODELS[1]), allow_custom_value=True, visible=True, scale=5)
                        refresh_models_btn = gr.Button("🔄", size="sm", visible=False, scale=0, min_width=50) # Only local
                        chunk_size_input = gr.Number(label=i18n("Chunk Size"), value=get_config("chunk_size", 70000), precision=0, scale=2)
                    ...
                    model_input = gr.Dropdown(["tiny", "small", "medium", "large", "large-v1", "large-v2", "large-v3", "turbo", "large-v3-turbo", "distil-large-v2", "distil-medium.en", "distil-small.en", "distil-large-v3"], label=i18n("Whisper Model"), value=get_config("whisper_model", "large-v3-turbo"))
                    with gr.Row():
                        workflow_input = gr.Dropdown(choices=[(i18n("Full"), "Full"), (i18n("Cut Only"), "Cut Only"), (i18n("Subtitles Only"), "Subtitles Only")], label=i18n("Workflow"), value=get_config("workflow", "Full"))
                        face_model_input = gr.Dropdown(["insightface", "mediapipe"], label=i18n("Face Model"), value=get_config("face_model", "insightface"))
                    with gr.Row():
                        face_mode_input = gr.Dropdown(choices=[(i18n("Auto"), "auto"), ("1", "1"), ("2", "2")], label=i18n("Face Mode"), value=get_config("face_mode", "auto"))
                        face_detect_interval_input = gr.Textbox(label=i18n("Face Det. Interval"), value=get_config("face_interval", "0.17,1.0"))
                        no_face_mode_input = gr.Dropdown(choices=[(i18n("Padding (9:16)"), "padding"), (i18n("Zoom (Center)"), "zoom")], label=i18n("No Face Fallback"), value=get_config("no_face_mode", "zoom"))
                    
                    
                    # Update listeners now that all components are defined
                    input_source.change(on_source_change, inputs=input_source, outputs=[url_input, time_interval_row, project_selector, video_upload, workflow_input])
             ...
             with gr.Row():
                 ...
                 save_cfg_btn = gr.Button("💾 " + i18n("Save Settings"), variant="secondary")
                 stop_btn = gr.Button("🛑 " + i18n("Stop Processing"), variant="stop", visible=False)
             stop_btn.click(kill_process, outputs=[])
             
             # Save Config Logic
             def save_current_config(seg, vir, thm, mdur, mxdr, aib, apik, aim, chnk, whisp, wrkf, fmod, fmd, f_int, nfmd):
                 cfg = {
                     "segments": seg, "viral": vir, "themes": thm, "min_dur": mdur, "max_dur": mxdr,
                     "ai_backend": aib, "api_key": apik, "ai_model": aim, "chunk_size": chnk,
                     "whisper_model": whisp, "workflow": wrkf, "face_model": fmod,
                     "face_mode": fmd, "face_interval": f_int, "no_face_mode": nfmd
                 }
                 try:
                     with open(WEBUI_CONFIG_PATH, "w", encoding="utf-8") as f:
                         json.dump(cfg, f, indent=4)
                     return i18n("Settings saved successfully!")
                 except Exception as e:
                     return i18n(f"Error saving: {e}")
             
             logs_output = gr.Textbox(label=i18n("Logs"), lines=10, autoscroll=True, elem_id="logs_output")
             
             # Save button routing to visible logs:
             save_cfg_btn.click(save_current_config, inputs=[
                 segments_input, viral_input, themes_input, min_dur_input, max_dur_input,
                 ai_backend_input, api_key_input, ai_model_input, chunk_size_input,
                 model_input, workflow_input, face_model_input, face_mode_input, face_detect_interval_input, no_face_mode_input
             ], outputs=logs_output)
             ...
             start_btn.click(run_viral_cutter, inputs=[
                 input_source, project_selector, url_input, start_time_input, end_time_input, video_upload, segments_input, viral_input, themes_input, min_dur_input, max_dur_input, 
