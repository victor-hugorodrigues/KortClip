...
def test_encoder(encoder_name, preset):
    """Teste real se o encoder funciona codificando 1 frame preto."""
    cmd = [
        'ffmpeg', '-y', '-f', 'lavfi', '-i', 'color=c=black:s=16x16:d=1',
        '-c:v', encoder_name, '-preset', preset, '-f', 'null', '-'
    ]
    try:
        # Se retornar 0, o encoder suporta o hardware atual (ou software)
...
def get_best_encoder():
    ...
    try:
        ...
        if "h264_nvenc" in output:
            if test_encoder("h264_nvenc", "fast"):
                ...
                return CACHED_ENCODER
                 ...
                 print("NVIDIA Encoder found but test failed. Hardware not supported.")
        
        if "h264_amf" in output:
            if test_encoder("h264_amf", "speed"):
                print("Encoder Detected: AMD (h264_amf)")
                CACHED_ENCODER = ("h264_amf", "speed") # quality, speed, balanced
                return CACHED_ENCODER
            else:
                print("AMD Encoder found but test failed.")
            
        if "h264_qsv" in output:
             if test_encoder("h264_qsv", "veryfast"):
                 print("Encoder Detected: Intel QSV (h264_qsv)")
                 CACHED_ENCODER = ("h264_qsv", "veryfast")
                 return CACHED_ENCODER
             else:
                 print("Intel Encoder found but test failed.")
             
        # Mac OS (VideoToolbox)
        if "h264_videotoolbox" in output:
             if test_encoder("h264_videotoolbox", "default"):
                 print("Encoder Detected: MacOS (h264_videotoolbox)")
                 CACHED_ENCODER = ("h264_videotoolbox", "default")
                 return CACHED_ENCODER
             else:
                 print("MacOS Encoder found but test failed.")
