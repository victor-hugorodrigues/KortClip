...
def cut(segments, project_folder="tmp", skip_video=False):
    ...
    def test_encoder(encoder_name, preset="fast"):
    ...
    def generate_segments(response, project_folder, skip_video):
        if not check_nvenc_support():
            print("NVENC is not supported or test failed on this system. Falling back to libx264.")
            video_codec = "libx264"
        else:
        ...
        for i, segment in enumerate(segments):
            ...
            if not skip_video:
                ...
                try:
                    ...
                    if os.path.exists(output_path):
                        print(f"Removing corrupted output file: {output_path}")
                        try:
