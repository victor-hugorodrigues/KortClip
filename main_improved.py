...
def main():
    ...
    parser.add_argument("--start-time", help="Start time for video cropping (e.g. 00:01:30 or 90)")
    parser.add_argument("--end-time", help="End time for video cropping (e.g. 00:02:45 or 165)")
    ...
    if not args.skip_prompts and url and not start_time and not end_time:
        print(i18n("\nDo you want to process a specific interval of the video?"))
        use_interval = input(i18n("Process specific interval? (y/n) [default: n]: ")).strip().lower()
        if use_interval in ['y', 'yes']:
            start_time = input(i18n("Start time (e.g., 00:01:30 or 90) [default: 0]: ")).strip()
            if not start_time: start_time = "0"
            end_time = input(i18n("End time (e.g., 00:02:45 or 165) [default: end]: ")).strip()
    ...
    try:
        ...
        if not input_video:
            ...
            download_result = download_video.download(url, download_subs=download_subs, quality=args.video_quality, start_time=start_time, end_time=end_time)
        ...
        if workflow_choice == "3":
            ...
            if os.path.exists(cuts_folder):
                existing_files = [f for f in os.listdir(cuts_folder) if f.endswith('.mp4')]
                valid_cuts = []
                for f in existing_files:
                    path = os.path.join(cuts_folder, f)
                    if os.path.getsize(path) > 1024:  # At least 1KB to be considered valid video
                        valid_cuts.append(f)
                    else:
                        print(i18n("Found corrupted/empty cut file: {} (removing...)").format(f))
                        try:
                            os.remove(path)
                        except:
                            pass
                ...
                if valid_cuts:
                    print(i18n("\nExisting valid cuts found in: {}").format(cuts_folder))
                if args.skip_prompts:
                    cut_again_resp = 'no'
                else:
